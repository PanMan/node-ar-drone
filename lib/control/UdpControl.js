...
function UdpControl(options) {
  ...
  this._ips        = options.ips || [] //Allow multiple IP's to send to, by sending an array of ips, instead of one
...
UdpControl.prototype._sendCommands = function(commands) {
  ...
  if (this._ips.length){
    //We have multiple IP's, send to each.
    var that=this;
    this._ips.forEach(function (ip, index, array) {
      console.log('Sending to ip: ',ip);
      that._socket.send(buffer, 0, buffer.length, that._port, ip);
    });
  } else{
    //Only one IP, send to that
